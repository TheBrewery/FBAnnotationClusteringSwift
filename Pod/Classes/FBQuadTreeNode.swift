...
public class FBQuadTreeNode: NSObject {
    ...
    class func FBBoundingBoxContainsCoordinate(box: FBBoundingBox, coordinate: CLLocationCoordinate2D) -> Bool {
        let containsX = (box.x0 <= CGFloat(coordinate.latitude)) && (CGFloat(coordinate.latitude) <= box.xf)
        let containsY = (box.y0 <= CGFloat(coordinate.longitude)) && (CGFloat(coordinate.longitude) <= box.yf)
